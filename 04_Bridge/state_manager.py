...
# BUGFIX: on Windows, os.replace() can briefly deny read access to the
# destination file to any other handle open at that exact instant — unlike
# POSIX rename(), which swaps atomically regardless of open readers. With
# 5 concurrent async agents reading/writing this same file, a load_state()
# call can land in that split-second window and raise PermissionError
# (errno 13), which was silently falling back to default state (and once
# caused Telegram alerts to show "AQI 0"). These races clear in milliseconds,
# so a couple of short retries resolves it without changing any concurrency
# design.
_READ_RETRIES = 3
...
def load_state() -> dict:
    ...
    BUG FIX: transient Windows PermissionError from concurrent os.replace() is
              retried a few times before falling back to default state.
    """
    ...
    log.error("Cannot read bridge state file (%s) — returning default state", last_os_error)
    return _default()
...
def save_state(state: dict) -> bool:
    ...
    try:
        ...
        try:
            ...
            # but on Windows it can still raise PermissionError/WinError 5 if
            # another thread has the destination open for reading at that
            # exact instant. Same race as load_state()'s read retries — retry
            # a few times before giving up, since the window clears in ms.
            last_os_error: OSError | None = None
            for attempt in range(_READ_RETRIES):
                try:
